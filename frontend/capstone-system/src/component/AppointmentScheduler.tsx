...
export default function AppointmentScheduler({
  ...
  useEffect(() => {
    if (preSelectedId && patients.length > 0) {
      ...
      if (matched) {
        ...
        const displayName =
          patientMap.get(preSelectedId) || matched.name || preSelectedId;
  ...
  const standardTimeSlots = [
    { raw: "08:00:00", display: "08:00 AM" },
    { raw: "08:30:00", display: "08:30 AM" },
    { raw: "09:00:00", display: "09:00 AM" },
    { raw: "09:30:00", display: "09:30 AM" },
    { raw: "10:00:00", display: "10:00 AM" },
    { raw: "10:30:00", display: "10:30 AM" },
    { raw: "11:00:00", display: "11:00 AM" },
    { raw: "11:30:00", display: "11:30 AM" },
    { raw: "12:00:00", display: "12:00 PM" },
    { raw: "12:30:00", display: "12:30 PM" },
    { raw: "13:00:00", display: "01:00 PM" },
    { raw: "13:30:00", display: "01:30 PM" },
    { raw: "14:00:00", display: "02:00 PM" },
    { raw: "14:30:00", display: "02:30 PM" },
    { raw: "15:00:00", display: "03:00 PM" },
    { raw: "15:30:00", display: "03:30 PM" },
    { raw: "16:00:00", display: "04:00 PM" },
    { raw: "16:30:00", display: "04:30 PM" },
    { raw: "17:00:00", display: "05:00 PM" },
    { raw: "17:30:00", display: "05:30 PM" },
  ];
  ...
  const loadSchedule = async () => {
    try {
      ...
      const approved = data.filter(
        (a: Appointment) => a.appointment_status !== "Requested",
      );
      const mobileReqs = data.filter(
        (a: Appointment) => a.appointment_status === "Requested",
      );
  ...
  const handleAcceptRequest = async (appointment: Appointment) => {
    ...
    if (
      ...
      alert(
        "Cannot accept request: The requested time slot has already passed today.",
      );
      return;
    }
    ...
    const patientName =
      patientMap.get(String(appointment.patient_id)) || appointment.patient_id;
    const selectedSlot = standardTimeSlots.find(
      (slot) => slot.raw.substring(0, 5) === targetTime,
    );
  ...
  const handleBookSlot = async (e: React.FormEvent) => {
    ...
    const patientName =
      selectedPatient?.name || patientMap.get(formPatientId) || formPatientId;
    const selectedSlot = standardTimeSlots.find(
      (slot) => slot.raw === formTime,
    );
  ...
  const handleSaveEdit = async (e: React.FormEvent) => {
    ...
    if (editDate === todayString && isSlotInPast(editDate, editTime)) {
      setEditError(
        "Cannot schedule a time slot that has already passed today.",
      );
      return;
    }
    ...
    const patientName =
      patientMap.get(String(editingAppointment.patient_id)) ||
      editingAppointment.patient_id;
    const selectedSlot = standardTimeSlots.find(
      (slot) => slot.raw === editTime,
    );
    ...
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 relative">
        ...
        <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm">
          ...
          <div className="grid grid-cols-7 gap-2 text-center">
            {calendarDays.map((dayObj, index) => {
              ...
              const hasBookings = takenSlots.some(
                (s) => s.date === dayObj.dateString,
              );
              ...
              return (
                <button
                  ...
                  onClick={() =>
                    !isPast && setSelectedViewDate(dayObj.dateString)
                  }
                  disabled={isPast}
        ...
        <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between border-b pb-4 mb-6 gap-4">
            <h3 className="text-sm font-bold text-slate-800 uppercase tracking-wider">
              Agenda for{" "}
              {new Date(selectedViewDate).toLocaleDateString(undefined, {
                weekday: "long",
                month: "short",
                day: "numeric",
              })}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            {standardTimeSlots.map((slot) => {
              const isBooked = takenSlots.some(
                (s) =>
                  s.date === selectedViewDate &&
                  s.time === slot.raw.substring(0, 5),
              );
              ...
              return (
                <div
                  key={slot.raw}
                  className={`p-3 rounded-xl border flex items-center justify-between ${isBooked || isPastTime ? "bg-amber-50/60 border-amber-200/60" : "bg-slate-50 border-slate-100"}`}
                >
                  <div className="flex items-center space-x-3">
                    <span
                      className={`text-xs font-bold px-2 py-1 rounded ${isBooked || isPastTime ? "bg-amber-100 text-amber-800" : "bg-white border text-slate-600"}`}
                    >
                      {slot.display}
                    </span>
                    ...
                    {booking ? (
                      <div>
                        ...
                        >
                          {patientMap.get(String(booking.patient_id)) ||
                            booking.patient_id}
                        </button>
                      ...
                      <span className="text-xs text-slate-400">
                        {isPastTime ? "Unavailable" : "Available"}
                      </span>
                    )}
                  </div>
                  {booking ? (
                    ...
                    <div
                      className={`w-2 h-2 rounded-full ${isBooked || isPastTime ? "bg-amber-400" : "bg-emerald-400"}`}
                    />
                  )}
                </div>
              );
      ...
      <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm h-fit sticky top-6">
        <button
          ...
          className="mb-4 w-full text-xs font-bold text-blue-600 bg-blue-50 py-2 rounded-lg hover:bg-blue-100 transition-colors"
        >
        ...
        <form onSubmit={handleBookSlot} className="space-y-4">
          ...
          {successMessage && (
            <div className="text-xs bg-emerald-50 text-emerald-600 p-2.5 rounded-lg border border-emerald-100">
              {successMessage}
            </div>
          )}
            ...
            >
              ...
              {standardTimeSlots.map((slot) => {
                const isTaken = takenSlots.some(
                  (s) =>
                    s.date === formDate && s.time === slot.raw.substring(0, 5),
                );
                ...
                return (
                  <option key={slot.raw} value={slot.raw} disabled={isDisabled}>
                    {slot.display}{" "}
                    {isTaken ? "(Reserved)" : isPastTime ? "(Unavailable)" : ""}
                  </option>
                );
              })}
            </select>
          ...
          <button
            type="submit"
            className="w-full bg-blue-600 text-white rounded-lg p-2 text-sm font-semibold hover:bg-blue-700 transition-colors"
          >
            Confirm Reservation
          </button>
        </form>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 backdrop-blur-sm p-4">
          <div className="bg-white w-full max-w-2xl rounded-xl border border-slate-200 shadow-xl overflow-hidden flex flex-col max-h-[85vh]">
            <div className="p-4 border-b border-slate-100 flex items-center justify-between bg-slate-50">
              <div>
                <h4 className="text-sm font-bold text-slate-800 flex items-center gap-2">
                  <Bell size={16} className="text-blue-600" /> Incoming Mobile
                  Bookings
                </h4>
                <p className="text-[11px] text-slate-400">
                  Review, approve, or clear queue requests.
                </p>
              </div>
              <button
            ...
            <div className="p-4 overflow-y-auto flex-1 space-y-2">
              {incomingRequests.length === 0 ? (
                ...
                incomingRequests.map((req) => {
                  const timeMatch = standardTimeSlots.find(
                    (s) =>
                      s.raw.substring(0, 5) ===
                      req.appointment_time?.substring(0, 5),
                  );
                  const displayTime = timeMatch
                    ? timeMatch.display
                    : req.appointment_time;
                  const patientDisplayName =
                    patientMap.get(String(req.patient_id)) || req.patient_id;

                  return (
                    <div
                      key={req.id || req.appointment_id}
                      className="p-3 rounded-lg border border-slate-100 bg-slate-50 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 text-xs"
                    >
                      <div>
                        ...
                        <div className="text-slate-500 mt-1 space-x-3">
                          <span>
                            <strong>Date:</strong>{" "}
                            {req.appointment_date?.split("T")[0]}
                          </span>
                          <span>
                            <strong>Requested Time:</strong> {displayTime}
                          </span>
                        </div>
            ...
            <div className="p-3 bg-slate-50 border-t border-slate-100 flex justify-end">
              <button
                onClick={() => setIsModalOpen(false)}
                className="text-xs font-semibold px-4 py-2 border border-slate-200 rounded-lg text-slate-600 hover:bg-white bg-slate-50 transition-colors"
              >
                Close Queue Panel
              </button>
            </div>
            ...
            <form onSubmit={handleSaveEdit} className="p-4 space-y-4">
              ...
              <div>
                ...
                >
                  {standardTimeSlots.map((slot) => {
                    const ownSlot =
                      editingAppointment.appointment_date?.split("T")[0] ===
                        editDate &&
                      editingAppointment.appointment_time?.substring(0, 5) ===
                        slot.raw.substring(0, 5);
                    const isTaken =
                      !ownSlot &&
                      takenSlots.some(
                    ...
                    return (
                      ...
                      >
                        {slot.display}{" "}
                        {isTaken ? "(Reserved)" : isPastTime ? "(Passed)" : ""}
                      </option>
                    );
                  })}
                </select>
              ...
              <div className="flex items-center justify-between pt-2">
                ...
                <div className="flex gap-2">
                  ...
                  <button
                    ...
                    className="text-xs font-semibold px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 disabled:opacity-50"
                  >
